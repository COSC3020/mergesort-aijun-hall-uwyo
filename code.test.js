...
// Test 1: Output of mergesort should match javascript built in sort
const testSort =
...
// Test 2: Check that elements in the output of mergesort are in ASCENDING order
const testAscendingOrder =
    jsc.forall("array nat", function(arr) {
        var sortedArray = mergesort(arr.slice());
...
const testHardcoded =
    jsc.forall(jsc.constant([5, 2, 9, 1, 5, 6]), function(arr) {
        var a1 = JSON.parse(JSON.stringify(arr));
        var a2 = [1, 2, 5, 5, 6, 9];
        return JSON.stringify(mergesort(a1)) === JSON.stringify(a2);
    });
